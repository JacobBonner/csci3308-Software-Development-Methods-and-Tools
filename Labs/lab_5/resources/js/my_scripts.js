...
function viewStudentStats(id, toggle){
	...
	if(toggle == 0){ // visibility to hidden, height to 0
		visibilityStatus = "hidden";
		heightValue = "0px";
	}
	else if(toggle == 1){ // visibility to visible, height to auto
		visibilityStatus = "visible";
		heightValue = "auto";
	}

	document.getElementById(id).style.visibility = visibilityStatus;
...
function loadStatsPage(){
	...
	for(rowCounter = 2; rowCounter<numRows; rowCounter++){
	...
	document.getElementById('losses').innerHTML = numRows - 2 - wins;
}
...
function loadPlayersPage(){
	var dropTable = document.getElementById('player_selector');
	
	var counter;
	for(counter=0; counter<players.length; counter++){
		//var idName = "player_" + (counter+1).toString(10);
		var currPlayer = document.createElement('a');
		currPlayer.setAttribute('href', '#');
		currPlayer.setAttribute('class', 'dropdown-item');
		currPlayer.setAttribute('onclick', 'switchPlayers(' + counter.toString() + ');');
		currPlayer.text = players[counter].name;
		dropTable.appendChild(currPlayer);
	}
}
...
function switchPlayers(playerNum){
	...
	// average passing, average rushing, and average receiving yards
